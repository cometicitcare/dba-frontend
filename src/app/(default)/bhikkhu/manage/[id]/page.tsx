...
function ManageBhikkhuInner({ params }: PageProps) {
  const editId = params.id; // <-- dynamic segment
  ...
  const [values, setValues] = useState<Partial<BhikkhuForm>>({
    ...bhikkhuInitialValues,
    br_cat: NOVICE_CATEGORY_CODE, // why: disabled in UI; seed something sensible
  });
  ...
  const effectiveSteps: Array<StepConfig<BhikkhuForm>> = useMemo(
    () => (reviewEnabled ? [...steps, { id: steps.length + 1, title: "Review & Confirm", fields: [] }] : steps),
    [steps, reviewEnabled]
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await _manageBhikku({ action: "UPDATE", payload: { br_regn: editId, data: payload } } as any);

      toast.success("Bhikkhu updated successfully.", {
        autoClose: 1200,
  ...
  // Display labels for review/autocomplete
  ...
  // Normalize API → form values + display strings
  const normalizeApi = useCallback((api: any) => {
    ...
    const formPatch: Partial<BhikkhuForm> = {
      br_cat: br_cat_code || NOVICE_CATEGORY_CODE,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // enrich parshawa display if we have static list
              ...
              <div className="px-4 md:px-10 py-6" ref={sectionRef}>
                ...
                <div className="flex flex-col md:flex-row gap-4 md:gap-0 md:justify-between md:items-center mt-8 pt-6 border-t border-slate-200">
                  ...
                  {currentStep < effectiveSteps.length ? (
                    <button onClick={handleNext} disabled={loading} className="flex items-center justify-center gap-2 px-6 py-2.5 bg-slate-700 text-white rounded-lg font-medium hover:bg-slate-800 transition-all disabled:opacity-70">
                      Next ›
                    </button>
                  ) : (
                    <button
