...
// ---- helpers (why: narrow unknown service shapes defensively)
type ApiOk<T> = { data?: T };
...
export default function Page() {
  ...
  const fetchLocationData = async () => {
    try {
      ...
      // why: _getLocationData() is typed loosely; guard before access
  ...
  const fetchDistricts = async () => {
    ...
    try {
      ...
      const rows: District[] = Array.isArray((result as any)?.data?.data)
        ? ((result as any).data.data as District[])
        : [];
      setDistricts(rows);
    } catch (err) {
  ...
  const fetchProvinces = async () => {
    ...
    try {
      ...
      const rows: Province[] = Array.isArray((result as any)?.data?.data)
        ? ((result as any).data.data as Province[])
        : [];
      setProvinces(rows);
    } catch (err) {
