...
const STATIC_NIKAYAS: NikayaHierarchy[] = Array.isArray((selectionsData as any)?.nikayas)
  ? ((selectionsData as any).nikayas as RawNikayaEntry[]).map((entry) => ({
      nikaya: entry.nikaya,
      parshawayas: (entry.parshawayas ?? []).map((p) => ({ code: p.code, name: p.name })),
    }))
  : [];
      ...
      <div className={`transition-all duration-300 pt-16 ${sidebarOpen ? "ml-64" : "ml-0"}`}>
        <main className="p-6">
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                  <div className="lg:col-span-2">
                    <label className="flex flex-col gap-1">
                      ...
                      <input
                        ...
                        onChange={(e) => setFilters((s) => ({ ...s, searchKey: e.target.value }))}
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {nikayaLoading ? (
                      <span className="text-sm text-gray-500">Loading Nikaya...</span>
                    ) : nikayaError ? (
          ...
          <div className="relative">
            <DataTable columns={columns} data={records} onEdit={handleEdit} onDelete={handleDelete} hidePagination />
            {loading && (
          ...
          <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="text-sm text-slate-600">
              {records.length
                ? `Showing ${(filters.page - 1) * filters.limit + 1} to ${(filters.page - 1) * filters.limit + records.length}`
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg border text-sm font-medium ${
                    filters.page <= 1 || loading ? "text-slate-400 border-slate-200" : "text-slate-700 hover:bg-slate-50"
                  }`}
                >
                  Previous
                </button>
                <span className="text-sm font-semibold text-slate-700">Page {filters.page}</span>
                <button
                  type="button"
                  onClick={() => handlePageChange(filters.page + 1)}
                  disabled={loading || !hasMoreResults}
                  className={`px-4 py-2 rounded-lg border text-sm font-medium ${
                    !hasMoreResults || loading ? "text-slate-400 border-slate-200" : "text-slate-700 hover:bg-slate-50"
                  }`}
                >
                  Next
                </button>
