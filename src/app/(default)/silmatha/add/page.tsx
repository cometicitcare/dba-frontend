// app/(default)/silmatha/add/page.tsx
"use client";
...
const SILMATHA_CATEGORY_CODE = "CAT01";
const SILMATHA_HIDDEN_FIELDS: Array<keyof BhikkhuForm> = [
  "br_nikaya",
  "br_parshawaya",
  "br_mahanayaka_name",
...
const SILMATHA_HIDDEN_FIELD_NAMES = new Set<string>(SILMATHA_HIDDEN_FIELDS as string[]);

export const dynamic = "force-dynamic";

function AddSilmathaPageInner() {
  ...
  const validateStep = (stepIndex: number): boolean => {
    ...
    for (const f of step.fields) {
      if (hideNikayaFields && SILMATHA_HIDDEN_FIELD_NAMES.has(String(f.name))) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (values.br_cat === SILMATHA_CATEGORY_CODE) {
        SILMATHA_HIDDEN_FIELDS.forEach((field) => {
          delete (payload as Record<string, unknown>)[field];
        });
      }

      await _manageSilmatha({ action: "CREATE", payload: { data: payload } } as any);

      // Success toast then redirect (delay allows user to see it; move ToastContainer to root for persistence across routes).
      toast.success(bhikkhuId ? "Silmatha record updated successfully." : "Silmatha record created successfully.", {
        autoClose: 1200,
  ...
  const hideNikayaFields = values.br_cat === SILMATHA_CATEGORY_CODE;
  ...
  useEffect(() => {
    if (!hideNikayaFields) return;
    setValues((prev) => {
      let changed = false;
      const next = { ...prev };
      const nextRecord = next as Record<string, string | undefined>;
      SILMATHA_HIDDEN_FIELDS.forEach((field) => {
        if (nextRecord[field] && nextRecord[field] !== "") {
          nextRecord[field] = "";
          changed = true;
        }
      });
    ...
    setDisplay((prev) => {
      ...
      const nextRecord = next as Record<string, string | undefined>;
      SILMATHA_HIDDEN_FIELDS.forEach((field) => {
        if (field in nextRecord && nextRecord[field] && nextRecord[field] !== "") {
          nextRecord[field] = "";
          changed = true;
        }
      });
              ...
              <div className="bg-gradient-to-r from-slate-700 to-slate-800 px-6 md:px-10 py-6">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-slate-300 text-sm">Provide all required information for CAT01 (Silmatha).</p>
                  </div>
                </div>
              </div>
                  ...
                  {!isReview && (
                    <div className={`grid grid-cols-1 ${gridCols} gap-5`}>
                      {current.fields.map((f) => {
                        ...
                        if (hideNikayaFields && SILMATHA_HIDDEN_FIELD_NAMES.has(id)) {
                          return null;
                        }
                          ...
                          return (
                            <div key={id}>
                              <BhikkhuCategorySelect
                                ...
                                label={f.label}
                                required={!!f.rules?.required}
                                value={values.br_cat ?? SILMATHA_CATEGORY_CODE}
                                disabled
                              />
                              {err ? <p className="mt-1 text-sm text-red-600">{err}</p> : null}
                            </div>
                        ...
                        <div key={s.id} className="border border-slate-200 rounded-xl p-4 md:p-5">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {s.fields.map((f) => {
                              ...
                              if (hideNikayaFields && SILMATHA_HIDDEN_FIELD_NAMES.has(key)) {
                                return null;
                              }
                              const shown =
