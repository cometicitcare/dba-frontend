    ...
    <div className={className}>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <div>
          ...
          <select className="w-full border rounded-md px-3 py-2" value={selection.provinceCode ?? ""} onChange={(e) => setSelection({ provinceCode: e.target.value || undefined, districtCode: undefined, divisionCode: undefined, gnCode: undefined })} disabled={disabled} required={required}>
            <option value="">Select Province</option>
            {provinces.map((p) => <option key={p.cp_code} value={p.cp_code}>{p.cp_name}</option>)}
          </select>
        </div>

        <div>
          <label className="block mb-1 text-sm font-medium">{labels.district}</label>
          <select className="w-full border rounded-md px-3 py-2" value={selection.districtCode ?? ""} onChange={(e) => setSelection({ districtCode: e.target.value || undefined, divisionCode: undefined, gnCode: undefined })} disabled={disabled || !currentProvince} required={required}>
            <option value="">Select District</option>
            {districts.map((d) => <option key={d.dd_dcode} value={d.dd_dcode}>{d.dd_dname}</option>)}
          </select>
        </div>

        <div>
          <label className="block mb-1 text-sm font-medium">{labels.division}</label>
          <select className="w-full border rounded-md px-3 py-2" value={selection.divisionCode ?? ""} onChange={(e) => setSelection({ divisionCode: e.target.value || undefined, gnCode: undefined })} disabled={disabled || !currentDistrict} required={required}>
            <option value="">Select Divisional Secretariat</option>
            {divisions.map((dv) => <option key={dv.dv_dvcode} value={dv.dv_dvcode}>{dv.dv_dvname}</option>)}
          </select>
        ...
        <div>
          ...
          <select className="w-full border rounded-md px-3 py-2" value={selection.gnCode ?? ""} onChange={(e) => setSelection({ gnCode: e.target.value || undefined })} disabled={disabled || !currentDivision || gnLoading || !gnDivisions.length} required={required}>
            <option value="">Select GN Division</option>
            {gnDivisions.map((g) => {
              const code = gnCodeOf(g);
              return <option key={code} value={code}>{g.gn_gnname}</option>;
            })}
          </select>
        </div>
      </div>
