...
export function validateField<T>(field: FieldConfig<T>, value: string | undefined, all: Partial<T>, today: string): string {
  ...
  const baseValue = typeof rawValue === "string" ? rawValue : String(rawValue);
  const trimmed = baseValue.trim();
  const rules = field.rules ?? {};
  if (field.type === "checkbox") {
    if (rules.required && trimmed !== "true") return "Required";
    return "";
  }
  const v = field.type === "date" ? toYYYYMMDD(trimmed) : trimmed;
